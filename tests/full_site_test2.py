...
# Define initial raw weights (before normalization)
raw_weights = {
    'MP': 15.0,    # Minutes per game
    'FG': 8.0,     # Field Goals Made per game
    'FGA': 8.0,    # Field Goals Attempted per game
    'FG%': 8.0,    # Field Goal Percentage
    '3P': 6.0,     # Three-Point Field Goals Made per game
    '3PA': 6.0,    # Three-Point Field Goals Attempted per game
    'FT': 5.0,     # Free Throws Made per game
    'FTA': 5.0,    # Free Throws Attempted per game
    'FT%': 5.0,    # Free Throw Percentage
    'ORB': 4.0,    # Offensive Rebounds per game
    'DRB': 4.0,    # Defensive Rebounds per game
    'TRB': 4.0,    # Total Rebounds per game
    'AST': 6.0,    # Assists per game
    'STL': 4.0,    # Steals per game
    'BLK': 4.0,    # Blocks per game
    'TOV': 4.0,    # Turnovers per game
    'PF': 4.0,     # Personal Fouls per game
    'PTS': 15.0    # Points per game
}
...
# Convert updated college player's stats into a weighted NumPy array for cosine similarity
    ...
    if os.path.exists(csv_file):
        ...
        if all(col in season_data.columns for col in stat_columns):
            ...
            if nba_positions:
                ...
                # Calculate cosine similarity between weighted college player stats and weighted NBA players' stats
                similarities = cosine_similarity(weighted_nba_stats, college_stats)
...
most_similar_players = most_similar_players.sort_values("Similarity (%)",ascending=False)
